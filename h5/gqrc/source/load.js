...
(function(){
	...
	if(isWX){
		if(location.host != 't.people.com.cn'){
			location.href = 'http://t.people.com.cn/h5/guoqing';
			return;
		};
	};
	//loader
	var loader = {
		...
		init : function(){
			...
			var dataurl = '/nmsgimage/wx/2015yuebing/' + dataId.substr(0, 10) + '/' + dataId + '.txt';
		...
		preview : function(data){
			...
			var html = '<div class="globalPackage"><div class="sound s_off"></div><div class="moveArr"></div></div><div class="scenery page_0' + this.style +  (this.firstItem && this.firstItem.src?' userBg':'') + '">' + (this.firstItem.src ? ('<div class="userImgBox"><canvas id="bgCanvas_first" class="userBgImg" data-exec="mobileshow.hideLoading();delete window.mobileshow;"></canvas><img src="' + this.firstItem.src + '" class="userImg" onload="showBlurBg(this)" style="top:' + (Math.round((viewHeight - (this.firstItem.height * viewWidth / this.firstItem.width)) * 0.4)) + 'px"></div>'):'<div class="bg"></div>') +'<div class="title" data-animationIn="fadeInDown 1s 0"></div><div class="subtitle" data-animationIn="fadeInUp 1.5s 0.5s">' + decodeURI(this.firstItem.txt).replace(/</g, '&lt;').replace(/([,，;；。])/g, '$1<br>') + '</div><div class="footer" data-animationIn="fadeInUp 1s 1s" data-animation="shine 2s 0 2"><div class="logo"></div></div></div>'+
	'</div>';
			...
			for (var i = 0; i < this.items.length; i++) {
				html += '<div class="scenery" data-autoNext="1000"><div class="userImgBox" data-animationIn="' + (animations[i % animations.length]) + '"><canvas id="bgCanvas_' + i + '" class="userBgImg"></canvas>' + '<img '+ (i==0?'':'data-') +'src="' + this.items[i]['src'] + '" class="userImg" onload="showBlurBg(this)" style="top:' + (Math.round((viewHeight - (viewWidth / this.items[i]['width'] * this.items[i]['height'])) * 0.4)) + 'px"></div>';
				if(this.items[i]['txt']){
					html += '<div class="userTxt" data-animationIn="' + (txtInAnimations[i % txtInAnimations.length]) + '">' + decodeURI(this.items[i]['txt']).replace(/</g, '&lt;').replace(/([,，;；。])/g, '$1<br>') + '</div>';
				};
				html += '</div>';
			};
			html += '<div class="scenery page_end_0' + this.style + '">'+
		'<div class="bg"></div>'+
			'<div class="toEditor"><a href="/h5/guoqing" class="btn_04" data-animation="wobble 3s 1s"></a>' + (isWX?('<a href="javascript:showShare()" class="btn_05" style="margin-left:1em;"></a>'):'') + '</div></div>';
