	...
	*/
	...
	if(isWX){
		if(location.host != 't.people.com.cn'){
			location.href = 'http://t.people.com.cn/wx/wuyi';
			return;
		};
	};
	var localCache = {
	...
	var editor = {
		...
		setStyle : function(s){
			...
			this.firstItem.imgUrl = this.firstItem.defaultSrc = this.firstItem.img.src = this.elementsPath + 'icon.png';

			Q('#lastPageImg').setAttribute('src' , this.elementsPath + 'icon_e_0' + this.style + '.png');
	...
	'</div>';
			...
			for (var i = 1; i < this.items.length; i++) {
				...
				if(this.items[i].txt){
					html += '<div class="userTxt" data-animationIn="' + (txtInAnimations[(i-1) % txtInAnimations.length]) + '" style="'+ (txtPos[(i-1) % txtPos.length]) +'">' + decodeURI(this.items[i].txt).replace(/</g, '&lt;').replace(/([,，;；。])/g, '$1<br>') + '</div>';
				};
				html += '</div>';
			};
	...
	'</div>';
		...
		uploadJSON : function(){
			...
			if(id){
				...
				showMessage('请稍候...');
				window.location.href = '/h5/wuyi/' + id;
				return;
			};
			...
			xhr.onreadystatechange = function(){
				if(xhr.readyState == 4 && xhr.status == 200){
					...
					if(rTxt && (/^\d+$/).test(rTxt)){
						...
						showMessage('请稍候...');
						//window.location.href = '/h5/wuyi/' + rTxt;
						setTimeout(function(){
							window.location.href = '/h5/wuyi/' + rTxt;
						}, 2000);
