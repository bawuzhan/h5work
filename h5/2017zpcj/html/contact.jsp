...
<script>
    ...
    $('#but>a').on("click",function(){
        if((!flag)||(!$name.value.trim())||(!$phone.value.trim())||(!$address.value.trim())){
            $('.delay>span').text('请完善您的信息');
            $('.delay').css('display','block')
            return;
        }else{
        	$.post('http://t.people.com.cn/addJkcjContact.action',
        		{
        		sn:$('#sn').val(),
        	...
        	function(response){
                if(response==1){
                	alert('提交成功');
                	window.location.href='<%=pathV3 %>/jkcj2017CJ.action?sn=<s:property value="sn" />'}
                else if(response==0){
                    alert('提交失败，请检查输入')
                }else{
