	...
	*/
	...
	if(isWX){
		if(location.host != 't.people.com.cn'){
			location.href = 'http://t.people.com.cn/wx/muqin';
			return;
		};
	};
	var localCache = {
			...
			if(file){
				try{
					...
					reader.onload = function(e){
						...
						_self._readFile(url, orientation);
					};
					reader.onerror = function(e){
						_self._readFile(url, 0);
					}
					reader.readAsBinaryString(file);
				}catch(e){
					this._readFile(url, 0);
				}
			}
		},
		_readFile : function(url, orientation){
			/* 生成图片
			* ---------------------- */
			var _self = this;
			var img = new Image();
			...
			img.onload = function(){
				...
				canvas = _self.zoomCanvas(canvas, 640, 640);
	...
	var editor = {
		page : 1,
		bgAudio : 'http://i0.peopleurl.cn/microblog-v3/h5/muqin/images/sound.' + audioType,
		firstItem : { //封面
			isFirst : true,
			isDefault : true,
			defaultSrc : '',
		...
		defaultTxtIndex : 0,
		elementsPath : 'http://i0.peopleurl.cn/microblog-v3/h5/muqin/images/',
		style : 1,
		items : [],
		limit : 7,
		init : function(){
		...
		preview : function(){
			...
			var html = '<div class="globalPackage"><div class="sound s_off"></div><div class="moveArr"></div></div><div class="scenery page_0' + this.style +  (this.firstItem.isDefault==false?' userScenery frameR':'') + '">' + (this.firstItem.isDefault==false ? ('<div class="userImgBox" style="top:' + (Math.round((viewHeight - (this.firstItem.img.height * viewWidth / this.firstItem.img.width)) * 0.4)) + 'px"><div class="imgFrameTop"></div><div class="imgFrameBtm"></div><div class="imgFrameTit"></div><img src="' + this.firstItem.img.src + '" class="userImg"></div>'):'<div class="bg"></div>') +'<div class="title" data-animationIn="fadeInRight 1.5s 0"></div><div class="footer" data-animationIn="fadeInUp 1s 1s" data-animation="shine 2s 0 2"><div class="logo"></div></div>'+
	'</div>';
			...
			for (var i = 1; i < this.items.length; i++) {
				html += '<div class="scenery userScenery'+ ((i-1) % 2 == 0 ? '' : ' frameR') +'" data-autoNext="1000"><div class="userImgBox" data-animationIn="' + (animations[(i-1) % animations.length]) + '" style="top:' + (Math.round((viewHeight - (this.items[i].img.height * viewWidth / this.items[i].img.width)) * 0.5)) + 'px">' + '<div class="imgFrameTop"></div><div class="imgFrameBtm"></div><div class="imgFrameTit"></div><img src="' + this.items[i].img.src + '" class="userImg"></div>';
				if(this.items[i].txt){
					html += '<div class="userTxt" data-animationIn="' + (txtInAnimations[(i-1) % txtInAnimations.length]) + '">' + (this.items[i].txt.indexOf('\n')===-1?this.items[i].txt.replace(/</g, '&lt;').replace(/([,，;；。])/g, '$1<br>'):this.items[i].txt.replace(/</g, '&lt;').replace(/\n/g, '<br>')) + '</div>';
				};
				html += '</div>';
			};
			html += '<div class="scenery page_end_0' + this.style + '">'+
		'<div class="bg"></div><div class="title" data-animationIn="fadeInDown 1s 0"></div><div class="footer" data-animationIn="fadeInUp 1s 1s" data-animation="shine 2s 0 1"><div class="logo"></div></div></div>'+
	'</div>';
		...
		uploadJSON : function(){
			...
			if(id){
				if(window.localStorage){
					localStorage['wyId'] = id;
				};
				showMessage('请稍候...');
				window.location.href = '/h5/muqin/' + id;
				return;
			};
			...
			xhr.onreadystatechange = function(){
				if(xhr.readyState == 4 && xhr.status == 200){
					...
					if(rTxt && (/^\d+$/).test(rTxt)){
						//成功
						if(window.localStorage){
							localStorage['wyId'] = rTxt;
						};
						localCache.set('json_' + m5, rTxt)
						showMessage('请稍候...');
						//window.location.href = '/h5/muqin/' + rTxt;
						setTimeout(function(){
							window.location.href = '/h5/muqin/' + rTxt;
						}, 2000);
			...
			xhr.open('POST', API, true);
			xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
			xhr.send('confDataStr=' + json + '&recordType=muqin&imageCount=' + this.items.length);
		}
	};
