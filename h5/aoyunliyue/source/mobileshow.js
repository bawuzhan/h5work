...
(function(){
	...
	var mobileshow = {
		...
		init: function(ele, preloadList){
			...
			this.initPage = parseInt(sessionStorage.getItem('page') || 0);
			if(preloadList && preloadList.length){
				...
				this.select(this.reindex?this.initPage:0);
			};
		...
		updateUrl : function(){
			...
			sessionStorage.setItem('page', this.index);
		},
		next : function(){
		...
		endMoveScen : function(f){
			...
			setTimeout(function(){
				...
				_self.nowScen.style[getHumpCssName('transition')] = CSS_HEAD_NAME + 'transform' + ' ' + _self.switchingTime + ' ease-out';
				_self.nowScen.style[getHumpCssName('transform')] = "translateY(0px)";
				if(_self.preScen){
					_self.preScen.style[getHumpCssName('transition')] = CSS_HEAD_NAME + 'transform' + ' ' + _self.switchingTime + ' ease-out';
