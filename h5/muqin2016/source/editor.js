		...
		try {
		  try {
			...
			try {
				...
				canvas.style['-webkit-filter'] = canvas.style['-ms-filter'] = canvas.style['filter'] = 'blur(' + radius + 'px)';
				return;
			}
		  }
		} catch(e) {
	...
	*/
	...
	var editor = {
		...
		init : function(){
			...
			this.addImage('http://i0.peopleurl.cn/microblog-v3/h5/muqin2016/images/p_01.jpg','http://i0.peopleurl.cn/microblog-v3/h5/muqin2016/images/icon_01.jpg', true);
			this.addImage('http://i0.peopleurl.cn/microblog-v3/h5/muqin2016/images/p_02.jpg','http://i0.peopleurl.cn/microblog-v3/h5/muqin2016/images/icon_02.jpg', true);
			this.addImage('http://i0.peopleurl.cn/microblog-v3/h5/muqin2016/images/p_03.jpg','http://i0.peopleurl.cn/microblog-v3/h5/muqin2016/images/icon_03.jpg', true);
			//this.setPage(1);
		},
		setStyle : function(s){
			this.style = s || 1;
			...
			Q('#lastPageImg').setAttribute('src' , this.elementsPath + 'icon_end.jpg');
		...
		addImage : function(img, smallBase64, isDefaImg){
			var _self = this;
		...
		preview : function(){
			...
			viewHeight = document.body.clientHeight;
			window.resetTop = function(img){
				if(img.height * s != viewHeight){
					img.style.top = Math.round((viewHeight - (img.height * viewWidth / img.width)) * 0.4) + 'px';
				}
			}
			...
			var txtInAnimations = ['fadeInRight 3s ', 'fadeInLeft 3s ', 'fadeInRight 3s ', 'fadeInLeft 3s '];
			var txts, ti;
			for (var i = 1; i < this.items.length; i++) {
				html += '<div class="scenery p_0' + i + '" data-autoNext="1000"><div class="userImgBox" data-animationIn="' + (animations[(i-1) % animations.length]) + '"><canvas id="bgCanvas_' + (i-1) + '" class="userBgImg"></canvas>' + '<img src="' + this.items[i].imgSrc + '" class="userImg" onload="showBlurBg(this);" style="top:' + (Math.round((viewHeight - (this.items[i].imgHeight * viewWidth / this.items[i].imgWidth)) * 0.5)) + 'px"></div>';
				if(this.items[i].txt){
					html += '<div class="userTxt" data-animationIn="' + (txtInAnimations[(i-1) % txtInAnimations.length]) + '">' + decodeURI(this.items[i].txt).replace(/</g, '&lt;').replace(/\n+/g, '<br>') + '</div>';
				};
				html += '</div>';
			};
			html += '<div class="scenery page_end_0' + this.style + '"><div class="bg"></div><div class="photo" data-animationIn="rollIn 1s 0"></div></div>';
		...
		submit : function(){
			...
			for (var i = 0; i < this.items.length; i++) {
				if(this.items[i].imgUrl){continue};
				total += this.items[i].imgSrc.length + this.items[i].smallBase64.length;
			};
			this.uploadImg();
		...
		change : function(item){
			...
			Q('.imgTxtLayer .it_pic img').setAttribute('src', item.imgSrc);
			...
			imageLoader.onadd = function(bigBase64, smallBase64){
				...
				bigImg.src = bigBase64;

				Q('.imgTxtLayer .it_pic img').setAttribute('src', bigBase64);
			};
		...
		uploadImg : function(){
			...
			for (var i = 0; i < this.items.length; i++) {
				if(!this.items[i].imgUrl){
					total += this.items[i].smallBase64.length + this.items[i].imgSrc.length;
				};
			};
			...
			xhr.onreadystatechange = function(){
				if(xhr.readyState == 4 && xhr.status == 200){
					...
					if(rTxt && (/^\d+$/).test(rTxt)){
						...
						sumSize += _self.items[_self.uploadIndex].smallBase64.length + _self.items[_self.uploadIndex].imgSrc.length;
			...
			function postImg(){
				...
				// 缓存
				if(item.smallBase64 && !item.imgUrl){
				...
				var data = 'dataStr=' + encodeURIComponent(item.imgSrc.substr(23)) + '&dataSmall=' + encodeURIComponent(item.smallBase64.substr(23));
				xhr.send(data);
			};
			postImg();
		},
		getJSON : function(){
			...
			for (var i = 1; i < this.items.length; i++) {
				...
				js += '{"src":"' + this.items[i].imgUrl + '","txt":"' + encodeURI(this.items[i].txt.replace(/</g, '&lt;') || '').replace(/(\\|")/g, '\\$1') + '","width":' + (this.items[i].imgWidth || this.items[i].img.width) +',"height":' + (this.items[i].imgHeight || this.items[i].img.height) +'}';
			};
			js += ']}';
			return js;
		},
		uploadJSON : function(){
			...
			if(id){
				...
				showMessage('请稍候...');
				window.location.href = '/h5/muqin2016/' + id;
				return;
			};
			...
			xhr.onreadystatechange = function(){
				if(xhr.readyState == 4 && xhr.status == 200){
					...
					if(rTxt && (/^\d+$/).test(rTxt)){
						//成功
						if(window.sessionStorage){
							sessionStorage['tempId'] = rTxt;
						};
						localCache.set('json_' + m5, rTxt)
						showMessage('请稍候...');
						//window.location.href = 'http://mblog.people.com.cn/h5/muqin2016/' + rTxt;
						setTimeout(function(){
							window.location.href = 'http://mblog.people.com.cn/h5/muqin2016/' + rTxt;
						}, 2000);
			...
			xhr.open('POST', API, true);
			xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
			xhr.send('confDataStr=' + json + '&recordType=muqin2016&imageCount=' + this.items.length);
		}
	};
