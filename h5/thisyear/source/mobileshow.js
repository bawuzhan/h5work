...
(function(){
	...
	var mobileshow = {
		...
		init: function(ele, preloadList){
			...
			//雾层
			this.fogEle = Q('.fog', this.ele);
			if(this.fogEle){
				this.fog.init(this.fogEle, s);
			}
	...
	mobileshow.fog = {
		init : function(ele, s){
			var _self = this, src;
			if(ele && ele.getAttribute('data-src')){
			...
			var __startEvent = function(e){
				...
				if(e.type == 'touchstart'){
					...
					x = e.touches[0].pageX;
					y = e.touches[0].pageY;
					_self.canvas.removeEventListener('mousedown', __startEvent, false);
				}else{
					x = e.pageX;
					y = e.pageY;
				};
			...
			this.canvas.addEventListener('touchstart', __startEvent, false);
			this.canvas.addEventListener('mousedown', __startEvent, false);
		},
		moveBegin: function(x, y, type) {
			...
			if(this.__moveEvent){
				document.removeEventListener('mousemove', this.__moveEvent, false);
				document.removeEventListener('touchmove', this.__moveEvent, false);
				document.removeEventListener('mouseup', this.__endEvent, false);
				document.removeEventListener('touchend', this.__endEvent, false);
			};
			...
			this.__endEvent = function(e){
				...
				document.removeEventListener('touchmove', _self.__moveEvent, false);
				document.removeEventListener('touchend', _self.__endEvent, false);
				document.removeEventListener('mousemove', _self.__moveEvent, false);
				document.removeEventListener('mouseup', _self.__endEvent, false);
			...
			if(type == 'touchstart'){
				document.addEventListener('touchmove', this.__moveEvent, false);
				document.addEventListener('touchend', this.__endEvent, false);
			}else{
				document.addEventListener('mousemove', this.__moveEvent, false);
				document.addEventListener('mouseup', this.__endEvent, false);
			}
			
			this.moving(x, y);
		},
		moving : function(x, y){
			this.c += Math.sqrt((this.preDot.x - x) * (this.preDot.x - x) + (this.preDot.y - y) * (this.preDot.y - y));
		...
		moveEndFn : function(){
			...
			if(this.c > 3000 * this.s){
				this.status = 'hide';
				mobileshow.touch = true;
				this.canvas.style.opacity = 0;
				setTimeout(function(){ //移除
					_self.ele.style.display = 'none';
					_self.ele.removeChild(_self.canvas);
				}, 1000);
			};
		},
		writeImg : function(src){
			var _self = this;
			var bgImg = new Image();
			bgImg.onload = function(){
				_self.ctx.drawImage(bgImg, 0, 0, _self.canvas.width, _self.canvas.height);
			};
			bgImg.src = src;
		}
	};
