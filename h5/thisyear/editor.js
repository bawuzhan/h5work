...
// 主内容
	...
	var imageLoader = {
		...
		init : function(){
			...
			// this.$fileRef.click(function(){
			// 	_self.$file.trigger('click');
			// });
	...
	var editor = {
		page : 1,
		bgAudio : 'http://i0.peopleurl.cn/microblog-v3/h5/thisyear/images/sound.' + audioType,
		imagePath : 'http://i0.peopleurl.cn/microblog-v3/h5/thisyear/images/',
		allDefaultTxt : [
			[
				'最美的风景不在远方，而在心上……',
				'每个人的一生，都该来一段说走就走的旅行。',
				'在年轻的时候，趁着轻狂的时候，离开栖息的城市，去到一个能令你内心安静的地方。',
				'沐浴一段属于自己的时光，找寻自己内心最真实的声音。',
				'会不会每个人都怀揣着这样一个梦，有一天，背上行囊，去旅行，去看尽世间的美景。',
				'人生就是一场旅行，就是在体味着不同的生活。',
				'不断地行走在路上，因为最美的风景就在路上。'
			],
			[
				'人世间有一种相遇，不是在路上，而是在心里。',
				'人世间有一种陪伴，不是在身边，而是在灵魂。',
				'人这一辈子摆脱不掉的就是一个情字，亲情、友情、爱情，剪不断的是亲情，少不了的是友情，放不下的是爱情。',
				'生活不是电影，不会有那么多的不期而遇。所以我们要微笑向暖，安之若素。',
				'要记住每一个对你好的人，因为他们本可以不这么做的。',
				'友谊由一个缘字开始，友情全凭一个信字延续，朋友全靠一个心字长久!',
				'有缘才能相遇，有心才能相知。'
			],
			[
				'快乐源于对于生命的热爱。一切痛苦和磨难都已经成为过去，只有对生命充满热爱，生活才会充满阳光和希望。',
				'生命是美好的，当一个生命依恋另一个生命时，相依为命，结伴而行，会感觉到世界真的很美好。',
				'生命源于母亲的孕育，它是上帝赋予我们每个人的珍贵财富。',
				'每当婴儿呱呱落地，就预示着生命来到人世，从此人生的脚步就开始迈出!',
				'在生命与灵魂融汇的瞬间，凝聚着的是美好、幸福，这瞬间所迸发出的光华值得我们用一生守候，永世珍藏。',
				'欣赏生命吧，让我们的心灵有一个沉静的时候，笑看生命中那些美丽的瞬间，那些无以名状的感动',
			],
			[
				'“为什么世界上有这么多好吃的东西?”“不是好吃的东西很多，而是对你来说什么都好吃。”',
				'吃货就是：开心的时候，吃好吃的庆祝一下;难过的时候，吃好吃的安慰一下;无聊的时候，吃好吃的消遣一下;愤怒的时候，吃好吃的发泄一下。',
			...
			[
				...
				'以后的日子还长，给自己一个微笑。',
				'不要沮丧，不要放弃。',
				'有些美好还没来，它们只是迟到了，但绝不会缺席。',
				'宠辱不惊来去无意，如此心宁静，优雅随之。',
				'坚持着，总有一天，活成自己曾经渴望的模样。'
			],
			[
				'难以忘记初次见你，一双迷人的眼睛。',
				'因为有你，幸福从未缺席。',
		...
		elementsPath : 'http://i0.peopleurl.cn/microblog-v3/h5/thisyear/images/',
		elements : [ //需要加载的元素
			['bg_10.jpg', 'bg_end_10.jpg', 'subtitle_01.png', 'title_01.png'],
			['bg_20.jpg', 'bg_end_20.jpg', 'subtitle_02.png', 'title_02.png'],
			['bg_30.jpg', 'bg_end_30.jpg', 'subtitle_03.png', 'title_03.png'],
			['bg_40.jpg', 'bg_end_40.jpg', 'subtitle_04.png', 'title_04.png'],
		],
		defaultTxtIndex : 0,
		endTxt : [
		...
		setStyle : function(s){
			this.style = s || 1;
			this.setPage(2);
			$('#firstPageImg').attr('src' , this.imagePath + 'chText_' + this.style + '.png');
			$('#lastPageImg').attr('src' , this.imagePath + 'tc_icon_n_' + this.style + '.jpg');
	...
	'</div>';
		...
		'<div class="bg_endTit" data-animationIn="fadeInRight 1.5s 0.5s"></div>'+
		'<div class="bg_endText" data-animationIn="fadeInRight 1.5s 1s"></div>'+
	'</div>';
			$('.slideshow').html(html);
			this.setPage(3);
		...
		getJSON : function(){
			var js = '{"style":' + this.style + ',"scrnery":[';
			for (var i = 0; i < this.items.length; i++) {
				if(i != 0){
					js += ',';
				}
		...
		uploadJSON : function(){
			var API = '/wx2015ConfUpload.action';
			var xhr = new XMLHttpRequest();
			showMessage('上传场景中...');
			...
			if(id){
				...
				showMessage('请稍候...');
				window.location.href = 'http://mblog.people.com.cn/h5/thisyear/' + id;
				return;
			};
			...
			xhr.onreadystatechange = function(){
				if(xhr.readyState == 4 && xhr.status == 200){
					...
					if(rTxt && (/^\d+$/).test(rTxt)){
						//成功
						if(window.localStorage){
							localStorage['thisyearId'] = rTxt;
						};
						localCache.set('json_' + md5, rTxt)
						showMessage('请稍候...');
						//window.location.href = '/wx/thisyear/' + rTxt;
						setTimeout(function(){
							window.location.href = '/h5/thisyear/' + rTxt;
						}, 2000);
