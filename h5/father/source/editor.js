	...
	*/
	...
	if(isWX){
		if(location.host != 't.people.com.cn'){
			location.href = 'http://t.people.com.cn/wx/father';
			return;
		};
	};
	var localCache = {
		...
		_readFile : function(url, orientation){
			...
			* ---------------------- */
			...
			img.onload = function(){
				...
				canvas = _self.zoomCanvas(canvas, 532, 532);
	...
	var editor = {
		...
		preview : function(){
			...
			var html = '<div class="globalPackage"><div class="sound s_off"></div><div class="moveArr"></div></div><div class="scenery page_0' + this.style +  (this.firstItem.isDefault==false?' userScenery_2':'') + '">' + (this.firstItem.isDefault==false ? ('<div class="ornament" data-animationIn="zoomIn 1s 1s"></div><div class="userPic"><img src="' + this.firstItem.img.src + '" alt=""></div>'):'<div class="bg"></div><div class="month"></div><div class="superman" data-animationIn="supermanIn 1.5s 0.5s" data-animation="supermanShake 5s 0 0"></div>') +'<div class="title" data-animationIn="fadeInDown 2s 0"></div><div class="footer" data-animationIn="fadeInUp 1s 2s" data-animation="shine 2s 0 2"><div class="logo"></div></div>'+
			'</div>';
			...
			for (var i = 1; i < this.items.length; i++) {
				html += '<div class="scenery userScenery_'+ ((i-1) % 2 == 0 ? '1' : '2') +'" data-autoNext="10000"><div class="ornament" data-animationIn="zoomIn 1s 1s"></div><div class="en" data-animationIn="fadeInDown 1s 1s"></div>'+((i-1) % 2 == 0 ?'<div class="pic_bg" data-animationIn="picBgAnim 1s 1s"></div>':'')+'<div class="userPic" data-animationIn="' + (animations[(i-1) % animations.length]) + '"><img src="' + this.items[i].img.src + '"></div>';
				if(this.items[i].txt){
	...
	'</div>';
		...
		uploadJSON : function(){
			...
			if(id){
				if(window.localStorage){
					localStorage['fqId'] = id;
				};
				showMessage('请稍候...');
				window.location.href = '/h5/father/' + id;
				return;
			};
			...
			xhr.onreadystatechange = function(){
				if(xhr.readyState == 4 && xhr.status == 200){
					...
					if(rTxt && (/^\d+$/).test(rTxt)){
						//成功
						if(window.localStorage){
							localStorage['fqId'] = rTxt;
						};
						localCache.set('json_' + m5, rTxt)
						showMessage('请稍候...');
						//window.location.href = '/h5/father/' + rTxt;
						setTimeout(function(){
							window.location.href = '/h5/father/' + rTxt;
						}, 2000);
			...
			xhr.open('POST', API, true);
			xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
			xhr.send('confDataStr=' + json + '&recordType=father&imageCount=' + this.items.length);
		}
	};
